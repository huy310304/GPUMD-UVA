...
# Extract the last part of the path (e.g., 'Run2')
run_dir = os.path.basename(current_dir)

# Extract the number from the 'RunX' format
dir_number = ''.join(filter(str.isdigit, run_dir))

# Generate file names based on the directory number
kappa_filename = f"kappa_{dir_number}_file"
nu_filename = f"shc_nu_{dir_number}_400_file"
kw_filename = f"shc_kw_{dir_number}_400_file"

print(kappa_filename, nu_filename, kw_filename)
...
l  = [42.67976316972, 42.6597354759144, 42.6562987549984]
Lx, Lz = l[0], l[2]
Ly = 4.26597354759144
V = Lx * Ly * Lz

T = 300 # Change Temp to 100K here
Fe = 3.5e-4 # Change Force to 1.4e-4 here

calc_spectral_kappa(shc, driving_force=Fe, temperature=T, volume=V)
shc['kw'] = shc['kwi'] + shc['kwo']
shc['K'] = shc['Ki'] + shc['Ko']
Gc = np.load('../../../ballistic/Gc_400.npy')
shc.keys()
...
shc_nu = shc['nu']
shc_kw = shc['kw']
np.save(nu_filename, shc_nu)
np.save(kw_filename, shc_kw)
