...
np.save("shc_nu_avg_300K_Optimized_F", shc_nu_avg)
np.save("shc_kw_avg_300K_Optimized_F", shc_kw_avg)
...
Gc = np.load('../../../../ballistic/Expand_Box_Size/Gc_400.npy') #CHANGE FILE NAME
#shc.keys()
...
np.save("lambda_i_avg_300K_Optimized_F", lambda_i_avg)
...
subplot(2,2,1)
loglog(shc_nu_avg[:num_of_pts], lambda_i_avg[:num_of_pts], linewidth=3)  # Use semilogx for log-linear scale
...
subplot(2,2,3)
set_fig_properties([gca()])
plot(shc_nu_avg[:num_of_pts], lambda_i_avg[:num_of_pts], linewidth=3)
#xlim([0, 50])
#gca().set_xticks(range(0,51,10))
#ylim([0, 6000])
...
subplot(2,2,4)
semilogx(shc_nu_avg[:num_of_pts], lambda_i_avg[:num_of_pts], linewidth=3)  # Use semilogx for log-linear scale
ylabel(r'$\lambda$($\omega$) (nm)')
xlabel(r'$\nu$ (THz)')
